...
# ── Core logic (không phụ thuộc GUI) ──────────────────────────────────────────
def _run_core(
    studio_v1, studio_v2,
    base_dirs,
    patch_v1, patch_v2,
    parallel_v1, trigger_v1,
    parallel_v2, trigger_v2,
    xlsx_path,
    log_fn=None,
    stop_event=None,
):
    """
    Chạy so sánh V1 vs V2 với các params cho trước.
    Xuất kết quả ra xlsx_path.
    Trả về (base_all_results, base_profiles) hoặc (None, None) nếu lỗi.
    """
    if log_fn is None:
        log_fn = log
    ...
    def run_one(bd, idx):
        ...
        try:
            ...
            output_v1 = os.path.join(output_dir, build_output_name(ver_v1, patch_v1, parallel_v1, trigger_v1))
            output_v2 = os.path.join(output_dir, build_output_name(ver_v2, patch_v2, parallel_v2, trigger_v2))
            os.makedirs(output_v1, exist_ok=True)
            os.makedirs(output_v2, exist_ok=True)

            log_fn(f"[Base Dir {idx}] Copying project...")
            ...
            log_fn(f"[Base Dir {idx}] Found {len(all_truss_files)} truss file(s).")
            if not all_truss_files:
                log_fn(f"[Base Dir {idx}] ❌ No .tdltruss files found, skipping.")
                return bd, [], {}
            ...
            while current_files:
                ...
                log_fn(f"[Base Dir {idx}] Building XML ({len(current_files)} file(s)){retry_label}...")
                ...
                log_fn(f"[Base Dir {idx}] Launching TrussStudio {ver_v1} & {ver_v2}{retry_label}...")
                restore_v1 = apply_and_restore_feature_flags(studio_v1, parallel_v1, trigger_v1)
                restore_v2 = apply_and_restore_feature_flags(studio_v2, parallel_v2, trigger_v2)
                try:
                ...
                while True:
                    ...
                    if time.time() - last_change_time >= NO_PROGRESS_TIMEOUT:
                        log_fn(f"[Base Dir {idx}] ⚠️ No progress for {NO_PROGRESS_TIMEOUT}s "
                               f"(v1={done_v1}/{expected}, v2={done_v2}/{expected})")
                        break
                    if time.time() - last_log >= 30:
                        log_fn(f"[Base Dir {idx}] Waiting... v1={done_v1}/{expected}, v2={done_v2}/{expected}")
                        last_log = time.time()
                ...
                if newly_retrying:
                    ...
                    if last_v2:
                        log_fn(f"[Base Dir {idx}] ⚠️ v2 stopped after {last_v2} (retry {retry_num}/{MAX_RETRY_PER_FILE}) — "
                               f"{len(newly_retrying)} file(s) not reached: {', '.join(sample)}{'...' if len(newly_retrying) > 5 else ''}")
                    else:
                        log_fn(f"[Base Dir {idx}] ⚠️ {len(newly_retrying)} file(s) not responded (retry {retry_num}/{MAX_RETRY_PER_FILE}): "
                               f"{', '.join(sample)}{'...' if len(newly_retrying) > 5 else ''}")

                if next_batch:
                    log_fn(f"[Base Dir {idx}] Retrying {len(next_batch)} file(s)...")
                else:
                    log_fn(f"[Base Dir {idx}] ✅ All file(s) done or marked.")
            ...
            log_fn(f"[Base Dir {idx}] Comparing...")
            ...
            for txt_name in actual_files:
                ...
                if txt_name in not_responded:
                    ...
                    log_fn(f"[Base Dir {idx}] [SKIP] {txt_name} — missing in v2 output")

            for txt_name in not_responded:
                if txt_name not in seen_txt:
                    all_results.append((txt_name, [{"section": "Not Responded", "diff_count": -1, "diff_pct": -1, "lines_v1": 0, "lines_v2": 0}]))

            log_fn(f"[Base Dir {idx}] Parsing truss profiles...")
            filenames = [f for f, _ in all_results]
            profiles  = _parse_profiles(bd, filenames)

            log_fn(f"[Base Dir {idx}] ✅ Done: {len(all_results)} file(s), {len(not_responded)} not responded.")
            return bd, all_results, profiles

        except Exception as e:
            log_fn(f"[Base Dir {idx}] ❌ ERROR: {e}")
            try:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(5, len(sorted_base_dirs))) as executor:
        ...
        for bd in sorted_base_dirs:
            ...
            time.sleep(40)  # delay 40s giữa mỗi base
        for future in concurrent.futures.as_completed(futures):
            try:
                bd_result, results, profiles = future.result()
                base_all_results[bd_result] = results
                base_profiles[bd_result]    = profiles
                if not results and not stop_event.is_set():
                    log_fn(f"⚠️ [{os.path.basename(bd_result)}] Failed or no results.")
            except Exception as e:
    ...
    return base_all_results, base_profiles
    ...
    def _run():
        try:
            ...
            _run_core(
                ...
                studio_v2=studio_v2,
                base_dirs=base_dirs,
                patch_v1=var_patch_v1.get(),
                patch_v2=var_patch.get(),
                parallel_v1=var_parallel_v1.get(),
                trigger_v1=var_trigger_v1.get(),
                parallel_v2=var_parallel_v2.get(),
                trigger_v2=var_trigger_v2.get(),
                xlsx_path=xlsx_path,
            )
...
if __name__ == "__main__":
    from CICD import auto_runner

    def auto_start():
        auto_runner.start(gui_refs)
...
# NOTE: auto_runner callbacks được inject vào sau khi gui_refs sẵn sàng
# Xem phần if __name__ == "__main__" đã được cập nhật bên dưới